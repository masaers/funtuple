...
namespace com { namespace masaers { namespace funtuple {
	namespace detail {
		...
		inline constexpr decltype(auto) apply_chain_helper(std::false_type, FunTuple&& funtuple, Result&& result) {
			return std::forward<Result>(result);
		}
		template<int At, int Step,typename FunTuple, typename X>
		inline constexpr decltype(auto) apply_chain_helper(std::true_type, FunTuple&& funtuple, X&& x) {
			return apply_chain_helper<At+Step, Step>(in_tuple<At+Step, FunTuple>(),
				                                       std::forward<FunTuple>(funtuple),
				                                       std::get<At>(funtuple)(std::forward<X>(x)));
		}
	...
	inline constexpr decltype(auto) apply_chain(FunTuple&& funtuple, Args&&... args) {
		return detail::apply_chain_helper<Start+Step, Step>(detail::in_tuple<Start+Step, FunTuple>(),
                                                  		    std::forward<FunTuple>(funtuple),
	...
	inline constexpr decltype(auto) apply_pipe(FunTuple&& funtuple, Args&&... args) {
		return apply_chain<0, 1>(std::forward<FunTuple>(funtuple), std::forward<Args>(args)...);
	}
	template<typename FunTuple, typename... Args>
	inline constexpr decltype(auto) apply_compose(FunTuple&& funtuple, Args&&... args) {
		return apply_chain<std::tuple_size<typename std::decay<FunTuple>::type>::value-1, -1>(std::forward<FunTuple>(funtuple), std::forward<Args>(args)...);
	}
